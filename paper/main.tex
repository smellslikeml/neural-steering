...
\begin{abstract}
We present a standalone toolkit for discovering and steering neuron-level circuits in large language models.
Building on Relevance Propagation (RelP) attribution in the neuron basis, we show that \emph{as few as 100--200 MLP neurons} form complete, faithful circuits for factual recall, syntactic agreement, and safety refusal.
...
Our toolkit and all experiments are released as a lightweight Python package.
\end{abstract}
...
In this paper, we present a standalone toolkit for neuron-level circuit discovery and steering.
Our contributions are:

\begin{enumerate}
    \item \textbf{A dependency-light toolkit.} A Python package (\texttt{neuron\_steer}) implementing RelP attribution, circuit selection, neuron-level steering, contrastive discovery, edge attribution, and faithfulness evaluation. No SAE training, no external interpretability libraries.
    ...
    \item \textbf{Hourglass architecture discovery.} Building on the edge attribution method of \citet{arora2025circuits}, we trace neuron-to-neuron information flow and find that circuits exhibit an hourglass topology with bottleneck neurons that serve as both major source hubs (high fan-out) and target hubs (high fan-in).

    \item \textbf{Universal neuron detection and super weight connection.} An automated procedure for detecting universal neurons that fire across all tasks, and a connection between these neurons and super weights \citep{yu2024superweights}---anomalously large weight magnitudes in early layers that act as task-agnostic infrastructure.
...
\emph{Benign prompts are unaffected}: ``What is the capital of France?'' still produces ``Paris'' under refusal ablation.
...
\subsection{ICL vs.\ Steering: Circuit Independence Test}
\label{sec:icl_steering}

\paragraph{Motivation.}
If in-context learning (ICL) and activation steering operate through independent substrates, their behavioral effects should combine additively and their circuits should be non-overlapping.
We designed a pre-registered experiment to test this mechanistically across 8 persona-based behavioral domains (pessimism, sycophancy, skepticism, dark triad, moral nihilism, overconfidence, risk tolerance, conspiracy).

\paragraph{Setup.}
For each persona, we measure log-odds of the persona-direction response across an ICL sweep ($N = 0$ to $64$ examples) and a CAA steering sweep ($m = -3$ to $3$).
We fit a Bayesian behavioral model:
$\text{LO} = a \cdot m + b + \gamma \cdot N^{1-\alpha}$
and test for an interaction term $\delta \cdot m \cdot N^{1-\alpha}$.
A persona passes the behavioral gate (GO) if: $R^2 > 0.85$ on held-out data, the Bayesian model outperforms linear ($\Delta\text{AIC} > 4$), and the interaction is non-significant (permutation test, 500 permutations, Bonferroni-corrected $\alpha = 0.05/8$).
For GO personas, we discover ICL circuits (top-200 neurons by $|t|$-statistic of coherent vs.\ frequency-matched ICL contrast) and CAA steering circuits independently, then measure overlap and cross-ablation.

\paragraph{Behavioral results.}
On Llama-3.1-8B, 1/8 personas (pessimism) passes the behavioral gate ($R^2 = 0.879$, $\Delta\text{AIC} = 182.8$, perm $p = 0.018 > 0.00625$ after Bonferroni).
The remaining 7 fail: 4 due to $R^2 < 0.85$, 2 due to significant interaction, 1 due to insufficient $\Delta\text{AIC}$.
On Qwen2.5-7B, 0/8 pass; all fail on $R^2 < 0.85$.
The behavioral model replicates for at most 1 of 8 persona domains tested.

\paragraph{Circuit overlap.}
For pessimism, the top-200 ICL and CAA circuits share zero neurons (Jaccard $= 0.000$ at $k = 50, 100, 200$).
Layer distributions are perfectly separated (KS $D = 1.0$, $p < 10^{-100}$): ICL neurons concentrate in layers 2--11 (140/200 in layer~2), while CAA neurons concentrate in layers 29--31.

\paragraph{Cross-ablation reveals single dissociation.}
Ablating the top-50 steering neurons shifts steering log-odds by $+6.99$ ($z = 110$ vs.\ layer-matched random controls).
Ablating the top-50 ICL neurons shifts ICL log-odds by only $-0.064$ ($z = -0.8$, not significant), accounting for 1.0\% of the total ICL behavioral effect.
This is a single dissociation, not the double dissociation that full functional independence would predict.

\paragraph{ICL injection fails.}
Injecting the top-200 ICL neuron activation offsets at $1\times$ scale shifts log-odds by $+0.22$---in the wrong direction.
The 200 neurons split 108 positive / 92 negative, producing near-complete sign cancellation.
The ICL circuit identified via frequency-matched contrast is not sufficient for the ICL effect.

\paragraph{Interpretation.}
The structural separation is real: zero neuron overlap, complete layer separation, additive behavioral composition (2.5\% residual).
But the ICL neurons we identify are not the causal mechanism of ICL.
The concentration of ``ICL circuit'' neurons in layer~2 (where they likely detect surface-level topic differences between persona questions and factual control questions, rather than abstract belief content) and their causal inertness under ablation both suggest that MLP neurons are not the primary substrate for ICL in this setting.
This is consistent with work identifying attention heads as the dominant ICL mechanism in models of this scale.
Future work should use same-domain contrasts (e.g., coherent vs.\ label-permuted ICL) to better isolate the causal ICL circuit in MLP neurons, if one exists.
...
The top-50 overlap between CAA and RelP is only 3--5 neurons out of 50.
...
An advantage of neuron-level steering over residual-stream methods (CAA, representation engineering) is \emph{specificity}.
...
We have not evaluated on reasoning tasks, multi-step behaviors, or diverse behavioral steering beyond the tasks reported here.

\textbf{ICL circuit discovery.}
Our ICL--steering independence test (Section~\ref{sec:icl_steering}) identifies ``ICL circuits'' via frequency-matched contrast, which differs in both topic domain and label structure from the coherent ICL condition.
The identified neurons may reflect surface-level input statistics (topic detection) rather than the abstract belief-update mechanism.
A same-domain contrast (coherent vs.\ label-permuted) would provide a cleaner control.
The 500-permutation test is also underpowered for the Bonferroni-corrected threshold ($\alpha = 0.00625$), and the sole GO persona (pessimism) depends on this correction for its passing status.
...
We have presented a toolkit for neuron-level circuit discovery and steering in large language models.
By implementing the three LRP rules (LN-rule, AH-rule, Half-rule) in a lightweight Python package, we make it possible to discover faithful circuits in a single forward-backward pass, steer model behavior by modifying ${\sim}200$ neurons (0.04\% of the model), and trace neuron-to-neuron information flow---all with zero external interpretability dependencies.
...
(4) Super weight neurons are detectable as universal infrastructure via edge attribution, providing the first connection between super weights and mechanistic circuit analysis.
(5) The entire approach generalizes across model families with zero code changes.
(6) A pre-registered ICL--steering independence test across 8 behavioral domains yields a negative result: while ICL-attributed and steering neurons occupy completely different layers (zero overlap, KS $D{=}1.0$), the identified ICL circuit is not causally effective ($z < 2$ under ablation, injection shifts log-odds in the wrong direction).
This suggests that MLP-based circuit discovery does not capture the ICL mechanism, and that structural circuit separation does not imply functional dissociation.

We release our toolkit as a Python package.
