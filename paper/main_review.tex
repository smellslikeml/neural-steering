...
We provide a clean, standalone reimplementation of their three LRP linearization rules (LN-rule, AH-rule, Half-rule) in a single-file Python module requiring only PyTorch and Hugging Face Transformers.
...
\begin{enumerate}
    \item \textbf{A standalone reimplementation.} A single-file Python module (\texttt{neuron\_steer.py}) implementing their RelP attribution, circuit selection, neuron-level steering, edge attribution, and faithfulness evaluation. No SAE training, no external interpretability libraries---only PyTorch and Hugging Face Transformers.
...
The primary value of this work is accessibility: a single-file Python module with no external interpretability dependencies that makes it easy to try neuron-basis circuit discovery on new tasks and models.
