...
\begin{abstract}
Recent work \citep{arora2025circuits} has shown that language model circuits are remarkably sparse in the neuron basis: roughly 100--200 MLP neurons, identified via Relevance Propagation (RelP), form faithful circuits for diverse tasks.
...
We extend the method with: (1)~a contrastive discovery mode that applies Contrastive Activation Addition at the neuron level, enabling targeted behavioral steering (e.g., disabling refusal while preserving general capabilities) for behaviors where clean target tokens are unavailable, (2)~an automated universal-neuron detection procedure that replaces Arora et al.'s hand-curated blacklist and makes the pipeline immediately portable to new models, and (3)~cross-model validation on Qwen2.5-7B, Mistral-7B, and Llama-3.2-1B with zero code changes, spanning 1B to 8B parameters.
...
\begin{enumerate}
    \item \textbf{A standalone reimplementation.} A Python package (\texttt{neuron\_steer}) implementing their RelP attribution, circuit selection, neuron-level steering, edge attribution, and faithfulness evaluation. No SAE training and no external interpretability libraries; only PyTorch and Hugging Face Transformers.
    ...
    \item \textbf{A contrastive discovery extension.} For behaviors without clean target tokens (e.g., refusal, tone), we introduce a contrastive mode that computes per-neuron activation differences between positive and negative prompt sets, applying the logic of Contrastive Activation Addition \citep{rimsky2024steering} at the neuron level. This enables targeted behavioral steering (e.g., disabling refusal) with high specificity, modifying only ${\sim}200$ neurons rather than the entire residual stream.

    \item \textbf{Automated blacklist detection.} \citet{arora2025circuits} provide a hand-curated blacklist of 12 universal neurons for Llama-3.1-8B. We automate this with a simple frequency-based procedure, which is useful when applying the method to new models.

    \item \textbf{Cross-model runs.} We confirm the approach works on Qwen2.5-7B, Mistral-7B, and Llama-3.2-1B with zero code changes, spanning model sizes from 1B to 8B parameters across three architecture families.

    \item \textbf{An hourglass observation.} Running their edge attribution method on our circuits, we observe a consistent bottleneck topology where information converges to and diverges from a single hub neuron.
\end{enumerate}
...
\citet{jafari2025relp} demonstrate that RelP achieves 0.956 Pearson correlation with activation patching on circuit discovery benchmarks (GPT-2 Large on the Indirect Object Identification task \citep{wang2023ioi}) while requiring only a single forward-backward pass.
...
These parameters are critical for model function: zeroing a single super weight column can collapse model output entirely.
...
The elementwise product of two input-dependent factors creates an attribution problem: how much credit should each factor receive?
...
\citet{arora2025circuits} provide a hard-coded blacklist for Llama-3.1-8B (12 neurons).
...
For new architectures (Qwen, Mistral, and smaller Llama variants), it provides a blacklist without manual analysis, making the method immediately portable to new models.
...
This provides a practical pathway for behaviors not easily expressed as single-token attribution targets, bridging the gap between residual-stream steering methods and neuron-level circuit analysis.
...
This demonstrates the practical utility of the neuron-level contrastive approach: unlike residual-stream CAA, which modifies entire activation vectors, neuron-level steering achieves the same behavioral shift while touching only 0.04\% of MLP neurons, preserving unrelated model capabilities.
...
We run our reimplementation on Qwen2.5-7B-Instruct, Mistral-7B-Instruct-v0.3, and Llama-3.2-1B-Instruct with \emph{zero code changes}.
The architecture-agnostic design (hooking \texttt{model.model.layers[i].mlp.down\_proj}) works across all four models, spanning 1B to 8B parameters.

\begin{table}[ht]
\centering
\caption{Cross-model runs across four models spanning 1B--8B parameters. P(``I'') measures refusal probability under circuit ablation.}
...
Llama-3.2-1B     & \checkmark & n/a$^\ddagger$  & n/a$^\ddagger$   & \checkmark \\
\bottomrule
...
\multicolumn{5}{l}{\footnotesize $^\ddagger$ Llama-3.2-1B has weaker refusal; circuit discovery and capitals steering confirmed with zero code changes.}
\end{tabular}
\end{table}
...
All four models use gated-MLP architectures ($\text{SiLU}(\text{gate}) \times \text{up}$), and the automated universal neuron detection (Section~\ref{sec:blacklist}) generates correct model-specific blacklists for each without manual intervention, confirming the portability of the full pipeline.
...
The connection between super weights and circuit-level edge attribution has not, to our knowledge, been explicitly discussed in prior work.
This suggests that super weight neurons may serve as task-agnostic computational infrastructure, consistently routing information to task-specific circuit components regardless of the downstream behavior.
...
Our contrastive extension inherits this advantage while extending the method to behaviors (refusal, tone, sycophancy) that cannot be expressed as single-token attribution targets.
...
We demonstrate compatibility across Llama (8B and 1B), Qwen, and Mistral families, all of which use gated-MLP architectures ($\text{SiLU}(\text{gate}) \times \text{up}$).
...
Beyond reproduction, we contribute contrastive discovery (CAA at the neuron level), automated universal neuron detection, and cross-model validation spanning 1B to 8B parameters across four model families.
...
For Llama-3.1-8B, the following 12 neurons are hard-coded from the TransluceAI codebase \citep{arora2025circuits} as universal (task-independent) and excluded from all circuits:
